...
// interactive question types belong in the bottom band, below the two columns.
const BOTTOM_VISUALS = new Set(["showMultipleChoice", "showSlider"]);

// where a jsCall piece auto-places: bottom for questions, right for textual
// visuals, left for everything else (diagrams). a teacher can override the whole
// act with addAct(..., { location }).
function autoZoneForJsCall(jsCall: string): "left" | "right" | "bottom" {
  if (BOTTOM_VISUALS.has(jsCall)) return "bottom";
  if (TEXT_VISUALS.has(jsCall)) return "right";
  return "left";
}

// one-line vizLib call: a target div plus the script that draws into it.
function vizCall(uid: string, jsCall: string, jsArgs: Record<string, any>): string {
  const vizId = `viz-${uid}`;
  const argsJson = JSON.stringify({ ...jsArgs, targetId: vizId });
  return `      <div id="${vizId}"></div>\n      <script>vizLib.${jsCall}(${argsJson});</script>`;
}

// render one act as a slideshow slide: a top-right act label, a two-column split
// (visual left / text right) and a bottom band for questions. each piece is placed
// by type — diagrams left (.act-visual), text / steps / math / highlights right
// (.act-text), MCQ / slider bottom (.act-bottom) — unless act.location overrides
// where the act's visual (jsCall + rawHtml) goes. text always stays right. a zone
// that gets no content isn't emitted, so a single-piece act flexes to fill.
//
// `index` makes the DOM ids unique per slide: a freeform lesson can visit the same
// act twice (a loop), and two slides sharing an id would make both vizLib scripts
// target the first one, leaving the second slide's visual empty.
export function assembleAct(act: ActResult, index = 0): string {
  const name = act.name || "act";
  const uid = `${htmlEscape(name)}-${index}`;
  const label = htmlEscape(name.replace(/_/g, " "));
  const parts: string[] = [
    `<div class="slide" id="slide-${uid}">`,
    `  <div class="act-label">${label}</div>`,
  ];

  const visualCol: string[] = [];
  const textCol: string[] = [];
  const bottomCol: string[] = [];
  const zoneFor = (zone: "left" | "right" | "bottom") =>
    zone === "left" ? visualCol : zone === "bottom" ? bottomCol : textCol;

  // text always lives in the right column (the explanation), never overridden.
  if (act.text) {
    textCol.push(`      <p>${htmlEscape(act.text).replace(/\n/g, "<br>")}</p>`);
  }
  // a jsCall piece: MCQ gets wrapped in .act-mcq; the location override (if any)
  // wins over the auto zone for this jsCall type.
  if (act.jsCall) {
    const zone = act.location ?? autoZoneForJsCall(act.jsCall);
    const html =
      act.jsCall === "showMultipleChoice"
        ? `      <div class="act-mcq">\n${vizCall(uid, act.jsCall, act.jsArgs)}\n      </div>`
        : vizCall(uid, act.jsCall, act.jsArgs);
    zoneFor(zone).push(html);
  }
  // rawHtml: latex (KaTeX) is math -> right; everything else (e.g. Desmos) -> left.
  // the location override wins here too.
  if (act.rawHtml) {
    const auto = act.rawHtml.includes('class="katex"') ? "right" : "left";
    zoneFor(act.location ?? auto).push(`      ${act.rawHtml}`);
  }

  const cols: string[] = [];
  if (visualCol.length) cols.push(`    <div class="act-visual">\n${visualCol.join("\n")}\n    </div>`);
  if (textCol.length) cols.push(`    <div class="act-text">\n${textCol.join("\n")}\n    </div>`);
  // only emit the two-column row when it has content, so a question-only act
  // doesn't leave an empty top half pushing the bottom band to the edge.
  if (cols.length) parts.push(`  <div class="act-columns">\n${cols.join("\n")}\n  </div>`);
  if (bottomCol.length) parts.push(`  <div class="act-bottom">\n${bottomCol.join("\n")}\n  </div>`);
...
export function assembleLesson(acts: ActResult[], title: string, template?: LessonTemplate): string {
  const slides = acts.map((act, i) => assembleAct(act, i)).join("\n");
...
<!-- web fonts used by some templates: Nunito (duolingo), Patrick Hand (chalk).
     other presets name system/near-system fonts and need no load. -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&family=Patrick+Hand&display=swap">
