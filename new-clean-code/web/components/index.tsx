...
const TextComp: ComponentFor = ({ intent, theme }) => {
  ...
  return (
    <div style={{ color, fontFamily: theme.font.body, lineHeight: 1.5 }}>
      <RichTextView value={it.content} theme={theme} />
    </div>
  );
};
  ...
  return (
    <div style={{ display: "grid", gap: theme.space(2) }}>
      <div style={{ color: theme.color.fg, fontFamily: theme.font.body, fontWeight: 600 }}>
        <RichTextView value={it.prompt} theme={theme} />
      </div>
      <div style={{ display: "grid", gap: theme.space(2) }}>
      ...
      {it.feedback ? (
        <div style={{ color: theme.color.muted, fontStyle: "italic" }}>
          <RichTextView value={it.feedback as RichText} theme={theme} />
        </div>
      ) : null}
      {answered ? (
        <button
  ...
  return (
    <div style={{ display: "grid", gap: theme.space(2) }}>
      ...
      >
        ...
        />
        <button type="submit" style={{ padding: `${theme.space(2)} ${theme.space(4)}`, background: theme.color.accent, color: theme.color.onAccent, border: "none", borderRadius: theme.radius, cursor: "pointer", fontWeight: 600 }}>
          Check
        </button>
      </form>
      {it.feedback ? (
        <div style={{ color: it.correct ? theme.color.correct : theme.color.muted, fontStyle: "italic" }}>
          <RichTextView value={it.feedback} theme={theme} />
        </div>
      ) : null}
      {answered ? (
...
const ControlsComp: ComponentFor = ({ intent, theme, send }) => {
  ...
  // Several buttons and nothing else — a detour's two ways out — are alternatives, so they read as
  // a row. Anything else keeps one control per line, which is what sliders and toggles want.
  const choices = it.controls.length > 1 && it.controls.every((c) => c.kind === "button");
  return (
    <div style={{ display: choices ? "flex" : "grid", flexWrap: "wrap", gap: theme.space(3) }}>
      {it.controls.map((c) => {
        if (c.kind === "button") {
          const event: MachineEvent = c.event
            ? { type: c.event }
            : c.key === "__next"
              ? { type: "next" }
              : { type: "demo.action", payload: { key: c.key } };
          return (
            <button
              key={c.key}
              onClick={() => send(event)}
              style={{ justifySelf: "start", padding: `${theme.space(2)} ${theme.space(4)}`, background: theme.color.accent, color: theme.color.onAccent, border: "none", borderRadius: theme.radius, cursor: "pointer", fontWeight: theme.font.weight.semibold, fontFamily: theme.font.body }}
            >
              {c.label}
            </button>
          );
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        color: on ? theme.color.onAccent : theme.color.fg,
