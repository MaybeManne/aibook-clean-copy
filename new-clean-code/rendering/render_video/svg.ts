...
import type { SceneNode, SceneSnapshot } from "@lessonkit/timeline";
...
function drawNode(n: SceneNode, theme: Theme, ctx: DefCtx): string {
  ...
  switch (n.kind) {
    ...
    case "label":
      return `<text${a} font-family="${esc(theme.font.body)}" font-size="${n.size ?? 32}" fill="${fill}" dominant-baseline="hanging">${esc(toPlain(n.text))}</text>`;
    case "group":
