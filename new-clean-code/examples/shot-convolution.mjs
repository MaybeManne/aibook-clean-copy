...
//   1. all 15 beats reach the screen in order (combine → dice → dice-formula → intro → flip →
...
const out = process.argv[3] ?? "/tmp/ls-convolution";
...
console.log(passed === results.length ? "BROWSER WALK PASS" : "BROWSER WALK FAIL");
await b.close();
