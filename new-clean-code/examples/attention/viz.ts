...
import { attentionRow, headOf, peakedness, tokens } from "./model.js";
...
registerViz("attention", (el: HTMLElement, initial: VizProps, api: VizApi): VizHandle => {
  ...
  const query = () => Math.max(0, Math.min(tokens.length - 1, Math.round(num(props.focus, 4))));
  const temp = () => num(props.temperature, 0.6);
  const head = () => headOf(!!props.positional);
  const cfgKey = () => `${head()}|${query()}|${temp().toFixed(2)}`;
  ...
  const colX = (i: number): number => {
    const pad = W * 0.09;
    return pad + (i * (W - 2 * pad)) / (tokens.length - 1);
  };
  const TOP_Y = () => H * 0.26;
  const BOT_Y = () => H * 0.74;
  ...
  function draw(): void {
    ...
    const q = query();
    const row = attentionRow(head(), q, temp());
    ...
    tokens.forEach((tok, j) => pill(colX(j), topY, tok, { hot: j === q ? 0 : row[j]! }));
    ...
    // HUD.
    const peak = peakedness(head(), q, temp());
    ...
    if (top) {
      ...
      ctx.fillText(`“${tokens[q]}” → “${tokens[top.j]}”  ${top.w.toFixed(2)}${peak < 0.34 ? "   (attention is spreading out)" : ""}`, 14, 30);
    }
  ...
  function recompute(): void {
    ...
    const key = cfgKey();
    if (peakedness(head(), query(), temp()) < 0.3) {
