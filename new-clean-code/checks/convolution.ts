...
import { A, B, convolve, diceWays } from "../examples/convolution/figures.js";
import { buildCombine, buildFlip, buildSlide } from "../examples/convolution/storyboards.js";
import { EDITOR_PRESETS, KERNELS, SOBEL_X, SOBEL_Y, convolve2d, kernelGain, matchPreset, pixelArtSprite, type Kernel } from "../examples/convolution/kernels.js";
...
/**
 * Count the ringed (active) cells in a grid figure.
 *
 * Derived from the theme rather than from a literal hex: the grids ring the active cell in
 * `figure.ink`, and a themed figure resolves that per theme. Keying the count to one hard-coded
 * colour would make this assertion pass only for the dark studio and silently count zero under any
 * other theme — which is exactly the bug it is supposed to catch.
 */
const ringOf = (theme: Theme): RegExp => new RegExp(`stroke="${theme.figure.ink.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`, "g");
const countHighlights = (svg: string, theme: Theme = defaultTheme): number => (svg.match(ringOf(theme)) ?? []).length;
...
idIn[0] = 200; idIn[1] = 10; idIn[2] = 40;
...
const sxOut = convolve2d(step, w, h, sobelX);
